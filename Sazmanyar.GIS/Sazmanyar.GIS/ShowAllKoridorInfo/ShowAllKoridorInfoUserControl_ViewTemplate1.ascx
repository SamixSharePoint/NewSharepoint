...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ShowAllKoridorInfoUserControl_ViewTemplate1.ascx.cs" Inherits="Sazmanyar.GIS.ShowAllKoridorInfo.ShowAllKoridorInfoUserControl_ViewTemplate1" %>
...
<script type="text/javascript" language="javascript">
    ...
    function RefershPishnahadInfo() {
        ...
        var control = document.getElementById('<%= cmbDepartment.ClientID %>');
        var selectedvalue = control.options[control.selectedIndex].value;
        var strDepartment = '';
        if (selectedvalue != 'همه موارد') {
            strDepartment = selectedvalue;
        }

        X.ajax({
            type: "POST",
            url: strCurrentWebUrl + "/_layouts/15/Sazmanyar.GIS/ApplicationPage_FetchingData.aspx/FillSugestion_ProjectNameDistinctInfoDataBaseForDepartment",
            data: "{'strDepartment':'" + strDepartment + "'}",
    ...
    function RefereshReportsLinks() {
        ...
        var NameProjeh = "";
        if (bVaziyatSelect == true) {
            NameProjeh = X('.NameProjeh').val();
        }
    ...
    function RefereshAllRoutInMap() {
        ...
        FetchRecursiveAllCoridorInfoFromDataBase(NameProjeh, NameDepartment, Condition, 100, 1);
    }

    function FetchRecursiveAllCoridorInfoFromDataBase(NameProjeh, NameDepartment, Condition, PageSize, PageIndex) {
        if (PageIndex == 1) {
        ...
        X.ajax({
            type: "POST",
            url: strCurrentWebUrl + "/_layouts/15/Sazmanyar.GIS/ApplicationPage_FetchingData.aspx/FetchAllCoridorInfoFromDataBase_NewCombine_Department_ByPaging",
            data: "{'NameProjeh':'" + NameProjeh + "' ,'NameDepartment':'" + NameDepartment + "' ,'Condition':'" + Condition + "' , 'PageSize':'" + PageSize + "', 'PageIndex':'" + PageIndex + "'}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (strHtmlOutput) {
                if (strHtmlOutput.d != '' && strHtmlOutput.d != null && strHtmlOutput.d.length > 0) {
                    ...
                    document.getElementById("divSearchCount").innerHTML = gisKoridorFilterSummary(NameProjeh, NameDepartment) + gisCountChips(gmarkers.length, gpolys.length, ggans.length);
                    document.getElementById("divSearchResult").innerHTML = divSearchResult_html;
                    FetchRecursiveAllCoridorInfoFromDataBase(NameProjeh, NameDepartment, Condition, PageSize, PageIndex + 1);

                }
                else {

                    if (PageIndex == 1) {
                        document.getElementById("divSearchCount").innerHTML = gisKoridorFilterSummary(NameProjeh, NameDepartment);
    ...
    function createClickableObject_Custom(ID, label, points, BaseColor, StationFrom_ID, StationFrom_Title, LatFrom, LongFrom, StationTo_ID, StationTo_Title, LatTo, LongTo, ObjAllRoutInfo) {
        ...
        if (points.length > 0 && LatFrom == 0 && LongFrom == 0) {
            ...
            GEvent.addListener(objmarker_StationFrom, "click", function () {
                ...
                map.panTo(objmarker_StationFrom.getPoint());
                map.setCenter(objmarker_StationFrom.getPoint());
                    ...
                    <td>
                        <div class="gis-topbar">
                            <div class="gis-fields">
                                ...
                                <div class="gis-field">
                                    <span class="gis-field-label">نام دپارتمان:</span>
                                    <asp:DropDownList ID="cmbDepartment" CssClass="cmbDepartment gis-select" onchange="RefershPishnahadInfo(this)" runat="server"></asp:DropDownList>
                                </div>
                                <div class="gis-field">
