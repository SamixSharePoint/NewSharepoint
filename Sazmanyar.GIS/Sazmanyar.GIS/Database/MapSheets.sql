/*
    ...
    پرکردن جدول:
      * وب‌پارت ShowAllMapSheets (حالت ادمین): فایل ZIP شامل shp/dbf/prj/cpg را می‌گیرد، می‌خواند و
...
IF OBJECT_ID(N'dbo.MapSheets', N'U') IS NOT NULL
    DROP TABLE dbo.MapSheets;
GO
...
(
    ...
    CONSTRAINT UQ_MapSheets_SheetNo CHECK (LEN(LTRIM(RTRIM(SheetNo))) > 0)
);
GO
...
SELECT
    ...
    p.StartDateJ, p.FinishDateJ, p.PlannedStartJ, p.PlannedFinishJ, p.TotalCost,
