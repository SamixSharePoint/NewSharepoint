...
(function (d) {
    function j(b, a, c) { a = "(" + c.replace(k, "\\$1") + ")"; return b.replace(RegExp(a, "gi"), "<strong>$1</strong>") } function g(b, a) {
        ...
        }; this.initialize(); this.setOptions(a); this.el.data("autoSazmanyarcomplete", this)
    } var k = RegExp("(\\/|\\.|\\*|\\+|\\?|\\||\\(|\\)|\\[|\\]|\\{|\\}|\\\\)", "g"); d.fn.autoSazmanyarcomplete = function (b, a) { var c; if ("string" == typeof b) { if (c = this.data("autoSazmanyarcomplete"), "function" == typeof c[b]) c[b](a) } else c = new g(this.get(0) || d("<input />"), b); return c }; g.prototype = { killerFn: null, initialize: function () {
        var b, a, c; b = this; a = Math.floor(1048576 * Math.random()).toString(16);
        c = "AutoSazmanyarcomplete_" + a; this.killerFn = function (a) { 0 === d(a.target).parents(".autoSazmanyarcomplete").size() && (b.killSuggestions(), b.disableKillerFn()) }; this.options.width || (this.options.width = this.el.width()); this.mainContainerId = "AutoSazmanyarcompleteContainter_" + a; d('<div id="' + this.mainContainerId + '" style="position:absolute;z-index:9999;"><div class="autoSazmanyarcomplete-w1"><div class="autoSazmanyarcomplete" id="' + c + '" style="display:none; width:300px;"></div></div></div>').appendTo("body"); this.container = d("#" + c); this.fixPosition();
