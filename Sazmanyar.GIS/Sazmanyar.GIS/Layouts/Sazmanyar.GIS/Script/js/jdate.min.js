...
function DateChangeFormatString(date) {
    var StrDate = date.toString();
    var In1 = StrDate.indexOf("/", 0);
    var In2 = StrDate.indexOf("/", In1 + 1);
    if (In1 < 3) {
        m_day = StrDate.substring(0, In1);
        m_month = StrDate.substring(In1 + 1, In2);
        m_year = StrDate.substring(In2 + 1, StrDate.length);
        date = Date.jalaliConverter.gregorianToJalali(Array(m_year, m_month, m_day))
        var dateS = date[0] + "/" + date[1] + "/" + date[2];
    }
    else {
        var In2 = StrDate.indexOf("/", In1 + 1);
        var m_year = StrDate.substring(0, In1);
        var m_month = StrDate.substring(In1 + 1, In2);
        var m_day = StrDate.substring(In2 + 1, StrDate.length);
        date = Date.jalaliConverter.gregorianToJalali(Array(m_year, m_month, m_day))
        var dateS = date[0] + "/" + date[1] + "/" + date[2];
    }
    return dateS;
